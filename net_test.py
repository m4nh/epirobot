...
class mono_net(nn.Module):  # vgg version
    def __init__(self, input_nc, output_nc):
        ...
        self.downconv_1 = self.conv_down_block3D(input_nc, 32, 3)
    ...
    def conv_down_block3D(self, in_dim, out_dim, kernal):
        conv_down_block = []
        conv_down_block += [
            nn.Conv3d(1, out_dim, kernel_size=(16, kernal, kernal), stride=2, padding=int((kernal - 1) / 2)),
            nn.BatchNorm3d(out_dim), nn.ELU()]  # h,w -> h,w
        # conv_down_block += [
        #     nn.Conv3d(out_dim, out_dim, kernel_size=(in_dim, kernal, kernal), stride=1, padding=int((kernal - 1) / 2)),
